...
Public Class fpr_kirimkenek
    ...
    Private Sub btload_Click(sender As System.Object, e As System.EventArgs) Handles btload.Click
        ...
        sql = String.Format(" {0} and kd_karyawan in (select kd_karyawan from ms_pegawai where bagian='KENEK')", sql)

        Dim periode As String = String.Format("{0} s.d {1}", convert_date_to_ind(ttgl.EditValue), convert_date_to_ind(ttgl2.EditValue))

        Dim fs As New fpr_insentif2 With {.WindowState = FormWindowState.Maximized, .sql = sql, .tgl = periode, .tipe = 2, .jenis_lap = ttipe.SelectedIndex}
        fs.ShowDialog()
