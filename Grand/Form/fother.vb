...
Public Class fother
    ...
    Private Sub opendata()
        ...
        Const sql As String = "select * from ms_alasan"
    ...
    Private Sub cari()
        ...
        sql = "select * from ms_alasan where "

        Select Case tcbofind.SelectedIndex
            Case 0 ' kode
                sql = String.Format("{0} tipe like '%{1}%'", sql, tfind.Text.Trim)
            Case 1 ' nama
                sql = String.Format("{0} alasan like '%{1}%'", sql, tfind.Text.Trim)
        End Select
    ...
    Private Sub hapus()
        ...
        If Integer.Parse(dv1(Me.BindingContext(bs1).Position)("noid").ToString) = 0 Then
            sql = String.Format("delete from ms_alasan where alasan='{0}' and tipe='{1}'", dv1(Me.BindingContext(bs1).Position)("alasan").ToString, dv1(Me.BindingContext(bs1).Position)("tipe").ToString)
        Else
            sql = String.Format("delete from ms_alasan where noid={0}", dv1(Me.BindingContext(bs1).Position)("noid").ToString)
        End If
        ...
        Try
            ...
            Clsmy.InsertToLog(cn, "btother", 0, 0, 1, 0, dv1(Me.BindingContext(bs1).Position)("alasan").ToString, dv1(Me.BindingContext(bs1).Position)("tipe").ToString, sqltrans)
    ...
    Private Sub tsadd_Click(sender As System.Object, e As System.EventArgs) Handles tsadd.Click
        Using fkar2 As New fother2 With {.StartPosition = FormStartPosition.CenterParent, .dv = dv1, .addstat = True, .position = 0}
            fkar2.ShowDialog()
        End Using
    End Sub
    ...
    Private Sub tsedit_Click(sender As System.Object, e As System.EventArgs) Handles tsedit.Click
        ...
        Using fkar2 As New fother2 With {.StartPosition = FormStartPosition.CenterParent, .dv = dv1, .addstat = False, .position = bs1.Position}
            fkar2.ShowDialog()
        End Using
