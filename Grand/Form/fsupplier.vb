...
Public Class fsupplier
    ...
    Private Sub opendata()
        ...
        Const sql As String = "select * from ms_supplier"
    ...
    Private Sub cari()
        ...
        sql = "select * from ms_supplier where "

        Select Case tcbofind.SelectedIndex
            Case 0 ' kode
                sql = String.Format("{0} kd_supp='{1}'", sql, tfind.Text.Trim)
            Case 1 ' nama
                sql = String.Format("{0} nama_supp like '%{1}%'", sql, tfind.Text.Trim)
        End Select
    ...
    Private Sub hapus()
        ...
        Dim sql As String = String.Format("delete from ms_supplier where kd_supp='{0}'", dv1(Me.BindingContext(bs1).Position)("kd_supp").ToString)
        ...
        Try
            ...
            Clsmy.InsertToLog(cn, "btsupp", 0, 0, 1, 0, dv1(Me.BindingContext(bs1).Position)("kd_supp").ToString, dv1(Me.BindingContext(bs1).Position)("nama_supp").ToString, sqltrans)
    ...
    Private Sub tsadd_Click(sender As System.Object, e As System.EventArgs) Handles tsadd.Click
        Using fkar2 As New fsupplier2 With {.StartPosition = FormStartPosition.CenterParent, .dv = dv1, .addstat = True, .position = 0}
            fkar2.ShowDialog()
        End Using
    End Sub
    ...
    Private Sub tsedit_Click(sender As System.Object, e As System.EventArgs) Handles tsedit.Click
        ...
        Using fkar2 As New fsupplier2 With {.StartPosition = FormStartPosition.CenterParent, .dv = dv1, .addstat = False, .position = bs1.Position}
            fkar2.ShowDialog()
        End Using
