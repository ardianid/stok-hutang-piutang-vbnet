...
Public Class fgroup
    ...
    Private Sub opendata()
        ...
        Const sql As String = "select * from ms_group"
    ...
    Private Sub cari()
        ...
        sql = "select * from ms_group where "

        Select Case tcbofind.SelectedIndex
            Case 0 ' kode
                sql = String.Format("{0} kd_group ='{1}'", sql, tfind.Text.Trim)
            Case 1 ' nama
                sql = String.Format("{0} nama_np like '%{1}%'", sql, tfind.Text.Trim)
        End Select
    ...
    Private Sub hapus()
        ...
        Dim sql As String = String.Format("delete from ms_group where kd_group='{0}'", dv1(Me.BindingContext(bs1).Position)("kd_group").ToString)
        ...
        Try
            ...
            Clsmy.InsertToLog(cn, "btgroup", 0, 0, 1, 0, dv1(Me.BindingContext(bs1).Position)("kd_group").ToString, dv1(Me.BindingContext(bs1).Position)("nama_np").ToString, sqltrans)
    ...
    Private Sub tsadd_Click(sender As System.Object, e As System.EventArgs) Handles tsadd.Click
        Using fkar2 As New fgroup2 With {.StartPosition = FormStartPosition.CenterParent, .dv = dv1, .addstat = True, .position = 0}
            fkar2.ShowDialog()
        End Using
    End Sub
    ...
    Private Sub tsedit_Click(sender As System.Object, e As System.EventArgs) Handles tsedit.Click
        ...
        Using fkar2 As New fgroup2 With {.StartPosition = FormStartPosition.CenterParent, .dv = dv1, .addstat = False, .position = bs1.Position}
            fkar2.ShowDialog()
        End Using
