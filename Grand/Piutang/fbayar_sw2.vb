...
Public Class fbayar_sw2
    ...
    Private Sub opengrid()
        ...
        Dim sql As String = String.Format("SELECT     trbayar_sw2.noid, trbayar_sw2.nobukti_sw, trsewa.netto, trbayar_sw2.jumlah, trsewa.tanggal " & _
            "FROM         trbayar_sw2 INNER JOIN " & _
            "trsewa ON trbayar_sw2.nobukti_sw = trsewa.nobukti " & _
             "WHERE trbayar_sw2.nobukti='{0}'", tbukti.Text.Trim)
    ...
    Private Sub isi()
        ...
        Dim sql As String = String.Format("SELECT     trbayar_sw.nobukti, trbayar_sw.tanggal, ms_toko.kd_toko, ms_toko.nama_toko, ms_toko.alamat_toko, trbayar_sw.note " & _
                "FROM         trbayar_sw INNER JOIN " & _
                "ms_toko ON trbayar_sw.kd_toko = ms_toko.kd_toko " & _
              "WHERE trbayar_sw.nobukti='{0}'", nobukti)
    ...
    Private Function cekbukti(ByVal cn As OleDbConnection, ByVal sqltrans As OleDbTransaction) As String
        ...
        Dim sql As String = String.Format("select max(nobukti) from trbayar_sw where YEAR(tanggal)='{0}' and MONTH(tanggal)='{1}' and nobukti like '%{2}%'", Year(ttgl.EditValue), Month(ttgl.EditValue), tahunbulan)
        ...
        Return String.Format("PSW.{0}{1}{2}", tahun, bulan, kbukti)
    ...
    Private Sub simpan()
        ...
        Try
            ...
            If addstat Then
                ...
                '1 . update faktur
                Dim sqlin_faktur As String = String.Format("insert into trbayar_sw (nobukti,tanggal,kd_toko,note,jumlah) values('{0}','{1}','{2}','{3}',{4})", tbukti.Text.Trim, _
                ...
                Clsmy.InsertToLog(cn, "btbayar_sw", 1, 0, 0, 0, tbukti.Text.Trim, convert_date_to_eng(ttgl.EditValue), sqltrans)

            Else

                '1. update faktur
                Dim sqlup_faktur As String = String.Format("update trbayar_sw set tanggal='{0}',kd_toko='{1}',note='{2}',jumlah={3} where nobukti='{4}'", convert_date_to_eng(ttgl.EditValue), tkd_supir.EditValue, tnote.Text.Trim, Replace(GridView1.Columns("jumlah").SummaryItem.SummaryValue, ",", "."), tbukti.Text.Trim)

                cmd = New OleDbCommand(sqlup_faktur, cn, sqltrans)
                cmd.ExecuteNonQuery()

                Clsmy.InsertToLog(cn, "btbayar_sw", 0, 1, 0, 0, tbukti.Text.Trim, convert_date_to_eng(ttgl.EditValue), sqltrans)
    ...
    Private Function simpan2(ByVal cn As OleDbConnection, ByVal sqltrans As OleDbTransaction) As String
        ...
        For i As Integer = 0 To dv1.Count - 1
            ...
            If addstat = True Then
                ...
                Dim sqlins As String = String.Format("insert into trbayar_sw2 (nobukti,nobukti_sw,jumlah) values('{0}','{1}',{2})", tbukti.Text.Trim, nobukti_sw, jumlah)
                Using cmins As OleDbCommand = New OleDbCommand(sqlins, cn, sqltrans)
                    cmins.ExecuteNonQuery()
                End Using

                Dim sqltrsewa As String = String.Format("update trsewa set jmlbayar=jmlbayar + {0} where nobukti='{1}'", jumlah, nobukti_sw)
            ...
            Else
                ...
                Dim sqlc As String = String.Format("select * from trbayar_sw2 where nobukti='{0}' and nobukti_sw='{1}'", tbukti.Text.Trim, nobukti_sw)
                ...
                If drdc.Read Then
                    If Not drdc("nobukti").ToString.Equals("") Then
                        ...
                        Dim sqltrsewa_old As String = String.Format("update trsewa set jmlbayar=jmlbayar - {0} where nobukti='{1}'", jumlahsebelum, nobukti_sw)
                ...
                If ada = False Then
                    ...
                    End Using
                Else
                    Dim sqlup As String = String.Format("update trbayar_sw2 set jumlah={0} where nobukti='{1}' and nobukti_sw='{2}'", jumlah, tbukti.Text.Trim, nobukti_sw)
    ...
    Private Sub btadd_Click(sender As System.Object, e As System.EventArgs) Handles btadd.Click
        Using fkar2 As New fbayar_sw3 With {.StartPosition = FormStartPosition.CenterParent, .dv = dv1, .addstat = True, .position = 0, .kdtoko = tkd_supir.Text.Trim}
            fkar2.ShowDialog(Me)
        End Using
    End Sub
    ...
    Private Sub btedit_Click(sender As System.Object, e As System.EventArgs) Handles btedit.Click
        ...
        Using fkar2 As New fbayar_sw3 With {.StartPosition = FormStartPosition.CenterParent, .dv = dv1, .addstat = False, .position = Me.BindingContext(dv1).Position, .kdtoko = tkd_supir.Text.Trim}
            fkar2.ShowDialog(Me)
        End Using
    ...
    Private Sub btdel_Click(sender As System.Object, e As System.EventArgs) Handles btdel.Click
        ...
        Else
            ...
            Try
                ...
                Dim sqltrsewa As String = String.Format("update trsewa set jmlbayar=jmlbayar - {0} where nobukti='{1}'", jumlah, nosewa)
                Using cmdtrsewa As OleDbCommand = New OleDbCommand(sqltrsewa, cn, sqltrans)
                    cmdtrsewa.ExecuteNonQuery()
                End Using
                ...
                Dim sqlbayar As String = String.Format("update trbayar_sw set jumlah=jumlah-{0} where nobukti='{1}'", jumlah, tbukti.Text.Trim)
                Using cmdbayar As OleDbCommand = New OleDbCommand(sqlbayar, cn, sqltrans)
                    cmdbayar.ExecuteNonQuery()
                End Using

                Dim sqldetail As String = String.Format("delete from trbayar_sw2 where noid={0}", dv1(Me.BindingContext(dv1).Position)("noid").ToString)
