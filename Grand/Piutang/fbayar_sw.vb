...
Public Class fbayar_sw
    ...
    Private Sub opendata()
        ...
        Dim sql As String = String.Format("SELECT     trbayar_sw.nobukti, trbayar_sw.tanggal, ms_toko.nama_toko, ms_toko.alamat_toko, trbayar_sw.note, trbayar_sw.sbatal,trbayar_sw.jumlah " & _
        "FROM         trbayar_sw INNER JOIN ms_toko ON trbayar_sw.kd_toko = ms_toko.kd_toko where trbayar_sw.tanggal >='{0}' and  trbayar_sw.tanggal <='{1}'", convert_date_to_eng(tglperiod1), convert_date_to_eng(tglperiod2))
    ...
    Private Sub cari()
        ...
        sql = String.Format("SELECT     trbayar_sw.nobukti, trbayar_sw.tanggal, ms_toko.nama_toko, ms_toko.alamat_toko, trbayar_sw.note, trbayar_sw.sbatal,trbayar_sw.jumlah " & _
        "FROM         trbayar_sw INNER JOIN ms_toko ON trbayar_sw.kd_toko = ms_toko.kd_toko where trbayar_sw.tanggal >='{0}' and  trbayar_sw.tanggal <='{1}'", convert_date_to_eng(tglperiod1), convert_date_to_eng(tglperiod2))

        Select Case tcbofind.SelectedIndex
            Case 0 ' nobukti
                sql = String.Format("{0} and trbayar_sw.nobukti like '%{1}%'", sql, tfind.Text.Trim)
            Case 1 ' tanggal
                ...
                sql = String.Format("{0} and trbayar_sw.tanggal='{1}'", sql, convert_date_to_eng(tfind.Text.Trim))
            Case 2 ' supp
                sql = String.Format("{0} and ms_toko.nama_toko like '%{1}%'", sql, tfind.Text.Trim)
            Case 3
                sql = String.Format("{0} and trbayar_sw.note like '%{1}%'", sql, tfind.Text.Trim)
        End Select
    ...
    Private Sub hapus()
        ...
        Dim sql As String = String.Format("update trbayar_sw set sbatal=1,alasan_batal='{0}' where nobukti='{1}'", alasan.ToUpper, dv1(Me.BindingContext(bs1).Position)("nobukti").ToString)
        ...
        Try
            ...
            If hapus_detail(cn, sqltrans) = True Then
                Clsmy.InsertToLog(cn, "btbayar_sw", 0, 0, 1, 0, dv1(Me.BindingContext(bs1).Position)("nobukti").ToString, dv1(Me.BindingContext(bs1).Position)("tanggal").ToString, sqltrans)
    ...
    Private Function hapus_detail(ByVal cn As OleDbConnection, ByVal sqltrans As OleDbTransaction) As Boolean
        ...
        Dim sql As String = String.Format("SELECT     trbayar_sw2.noid, trbayar_sw2.nobukti_sw,trbayar_sw.kd_toko, trbayar_sw2.jumlah " & _
            "FROM         trbayar_sw2 INNER JOIN " & _
            "trbayar_sw ON trbayar_sw2.nobukti = trbayar_sw.nobukti INNER JOIN " & _
            "ms_toko ON trbayar_sw.kd_toko = ms_toko.kd_toko where trbayar_sw2.nobukti='{0}'", nobukti)
        ...
        If drd.HasRows Then
            While drd.Read
                ...
                Dim jumlah As Double = Double.Parse(drd("jumlah").ToString)
                Dim nosewa As String = drd("nobukti_sw").ToString
                ...
                Dim sqltoko As String = String.Format("update ms_toko set piutangsewa=piutangsewa + {0} where kd_toko='{1}'", jumlah, kdtoko)
    ...
    Private Sub cekbatal_onserver()
        ...
        Try
            ...
            Dim sql As String = String.Format("select sbatal from trbayar_sw where nobukti='{0}'", dv1(bs1.Position)("nobukti").ToString)
    ...
    Private Sub tsadd_Click(sender As System.Object, e As System.EventArgs) Handles tsadd.Click
        Using fkar2 As New fbayar_sw2 With {.StartPosition = FormStartPosition.CenterParent, .dv = dv1, .addstat = True, .position = 0}
            fkar2.ShowDialog()
        End Using
    End Sub
    ...
    Private Sub tsedit_Click(sender As System.Object, e As System.EventArgs) Handles tsedit.Click
        ...
        Using fkar2 As New fbayar_sw2 With {.StartPosition = FormStartPosition.CenterParent, .dv = dv1, .addstat = False, .position = bs1.Position}
            fkar2.ShowDialog()
        End Using
    ...
    Private Sub tsprint_Click(sender As System.Object, e As System.EventArgs) Handles tsprint.Click
        ...
        Using fkar2 As New fpr_bayar_sw With {.nobukti = nobukti}
            fkar2.ShowDialog(Me)
        End Using
