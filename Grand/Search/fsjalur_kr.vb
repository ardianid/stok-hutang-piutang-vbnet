...
Public Class fsjalur_kr
    Private dvmanager As Data.DataViewManager
    Private dv1 As Data.DataView

    Private Sub cari()

        Dim sql As String = "select top 50 * from ms_jalur_kirim"

        If Not tfind.Text.Trim.Equals("") Then

            Select Case cb_criteria.SelectedIndex
                Case 0
                    sql = String.Format("{0} where  kd_jalur like '%{1}%'", sql, tfind.Text.Trim)
                Case 1
                    sql = String.Format("{0} where  nama_jalur like '%{1}%'", sql, tfind.Text.Trim)
            End Select
    ...
    Public ReadOnly Property get_KODE As String
        Get
            ...
            If dv1.Count <= 0 Then
                ...
                Exit Property
            End If
            Return dv1(Me.BindingContext(dv1).Position)("kd_jalur").ToString
        End Get
    End Property
