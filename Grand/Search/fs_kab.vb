...
Public Class fs_kab
    ...
    Private Sub cari()
        ...
        Dim sql As String = "select * from ms_kab"

        If Not tfind.Text.Trim.Equals("") Then

            Select Case cb_criteria.SelectedIndex
                Case 0
                    sql = String.Format("{0} where  kd_kab like '%{1}%'", sql, tfind.Text.Trim)
                Case 1
                    sql = String.Format("{0} where  nama_kab like '%{1}%'", sql, tfind.Text.Trim)
            End Select
