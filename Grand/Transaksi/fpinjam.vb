...
Public Class fpinjam
    ...
    Private Sub opendata()
        ...
        Dim sql As String = String.Format("SELECT trpinjam.nobukti, trpinjam.tanggal, trpinjam.tgl_keluar, trpinjam.kd_toko, ms_toko.nama_toko, ms_toko.alamat_toko, trpinjam.note, trpinjam.sbatal " & _
            "FROM  trpinjam INNER JOIN ms_toko ON trpinjam.kd_toko = ms_toko.kd_toko where trpinjam.tanggal >='{0}' and  trpinjam.tanggal <='{1}'", convert_date_to_eng(tglperiod1), convert_date_to_eng(tglperiod2))
    ...
    Private Sub cari()
        ...
        Select Case tcbofind.SelectedIndex
            Case 0 ' kode
                sql = String.Format("{0} and trpinjam.nobukti like '%{1}%'", sql, tfind.Text.Trim)
            Case 1 ' nama
                ...
                sql = String.Format("{0} and trpinjam.tanggal='{1}'", sql, convert_date_to_eng(tfind.Text.Trim))
            Case 2 ' tgl keluar
                If Not IsDate(tfind.Text.Trim) Then
                    MsgBox("Koreksi Tanggal....", vbOKOnly + vbExclamation, "Informasi")
                    Return
                ...
                sql = String.Format("{0} and trpinjam.tgl_keluar='{1}'", sql, convert_date_to_eng(tfind.Text.Trim))
            Case 3 ' outlet
    ...
    Private Sub hapus()
        ...
        Dim sql As String = String.Format("update trpinjam set sbatal=1,alasan_batal='{0}' where nobukti='{1}'", alasan.ToUpper, dv1(Me.BindingContext(bs1).Position)("nobukti").ToString)
        ...
        Try
            ...
            If hapus_detail(cn, sqltrans) = True Then
                ...
                Clsmy.InsertToLog(cn, "btpinjam", 0, 0, 1, 0, dv1(Me.BindingContext(bs1).Position)("nobukti").ToString, dv1(Me.BindingContext(bs1).Position)("tanggal").ToString, sqltrans)
    ...
    Private Function hapus_detail(ByVal cn As OleDbConnection, ByVal sqltrans As OleDbTransaction) As Boolean
        ...
        Dim sql As String = String.Format("select * from trpinjam2 where nobukti='{0}'", nobukti)
        ...
        If drd.HasRows Then
            While drd.Read
                ...
                If IsNumeric(drd("noid").ToString) Then
                    ...
                    ' historical toko
                    Dim hasiltok As String = Hist_PinjamSewa_Toko(kdtoko, kdbar, qtykecil, cn, sqltrans, False, True)
                    ...
                    Clsmy.Insert_HistBarang(cn, sqltrans, nobukti, tanggal, kdgud, kdbar, qtykecil, 0, "Pinjam Barang", "-", "BE XXXX XX")
    ...
    Private Sub cekbatal_onserver()
        ...
        Try
            ...
            Dim sql As String = String.Format("select sbatal from trpinjam where nobukti='{0}'", dv1(bs1.Position)("nobukti").ToString)
    ...
    Private Sub tsadd_Click(sender As System.Object, e As System.EventArgs) Handles tsadd.Click
        Using fkar2 As New fpinjam2 With {.StartPosition = FormStartPosition.CenterParent, .dv = dv1, .addstat = True, .position = 0}
            fkar2.ShowDialog()
        End Using
    End Sub
    ...
    Private Sub tsedit_Click(sender As System.Object, e As System.EventArgs) Handles tsedit.Click
        ...
        Using fkar2 As New fpinjam2 With {.StartPosition = FormStartPosition.CenterParent, .dv = dv1, .addstat = False, .position = bs1.Position}
            fkar2.ShowDialog()
        End Using
    ...
    Private Sub tsprint_Click(sender As System.Object, e As System.EventArgs) Handles tsprint.Click
        ...
        Using fkar2 As New fpr_pinjam With {.nobukti = nobukti}
            fkar2.ShowDialog(Me)
        End Using
    End Sub
End Class