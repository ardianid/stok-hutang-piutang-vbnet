...
Public Class fpinjam2
    ...
    Private Sub opengrid()
        ...
        Dim sql As String = String.Format("SELECT trpinjam2.noid, trpinjam2.nobukti,trpinjam2.kd_gudang, trpinjam2.kd_barang, ms_barang.nama_barang, trpinjam2.qty, trpinjam2.satuan, trpinjam2.qtykecil " & _
            "FROM trpinjam2 INNER JOIN ms_barang ON trpinjam2.kd_barang = ms_barang.kd_barang where trpinjam2.nobukti='{0}'", tbukti.Text.Trim)
    ...
    Private Sub isi()
        ...
        Dim sql As String = String.Format("SELECT trpinjam.nobukti, trpinjam.tanggal, trpinjam.tgl_keluar, trpinjam.kd_toko, ms_toko.nama_toko, ms_toko.alamat_toko, trpinjam.note " & _
            "FROM trpinjam INNER JOIN ms_toko ON trpinjam.kd_toko = ms_toko.kd_toko where trpinjam.nobukti='{0}'", nobukti)
    ...
    Private Function cekbukti(ByVal cn As OleDbConnection, ByVal sqltrans As OleDbTransaction) As String
        ...
        Dim sql As String = String.Format("select max(nobukti) from trpinjam where nobukti like '%PNJ.{0}%'", tahunbulan)
        ...
        Return String.Format("PNJ.{0}{1}{2}", tahun, bulan, kbukti)
    ...
    Private Sub simpan()
        ...
        Try
            ...
            If addstat Then
                ...
                '0. insert spm
                Dim sqlin As String = String.Format("insert into trpinjam (nobukti,tanggal,tgl_keluar,kd_toko,note) values('{0}','{1}','{2}','{3}','{4}')", _
                ...
                Clsmy.InsertToLog(cn, "btpinjam", 1, 0, 0, 0, tbukti.Text.Trim, ttgl.EditValue, sqltrans)
            ...
            Else
                ...
                '1. update rekap
                Dim sqlup As String = String.Format("update trpinjam set tanggal='{0}',tgl_keluar='{1}',kd_toko='{2}',note='{3}' where nobukti='{4}'", convert_date_to_eng(ttgl.EditValue), convert_date_to_eng(ttgl_mt.EditValue), _
                ...
                Clsmy.InsertToLog(cn, "btpinjam", 0, 1, 0, 0, tbukti.Text.Trim, ttgl.EditValue, sqltrans)
            End If
            ...
            If Not dv1(i)("noid").Equals(0) Then
                ...
                If addstat = False Then
                    If DateValue(tglmuat_old) <> DateValue(ttgl_mt.EditValue) Then
                        Clsmy.Insert_HistBarang(cn, sqltrans, tbukti.Text.Trim, tglmuat_old, kdgudang, kdbar, qtykecil, 0, "Pinjam Barang", "-", "BE XXXX XX")
                    End If
                End If

                Clsmy.Insert_HistBarang(cn, sqltrans, tbukti.Text.Trim, ttgl_mt.EditValue, kdgudang, kdbar, 0, qtykecil, "Pinjam Barang", "-", "BE XXXX XX")


            End If

            If dv1(i)("noid").Equals(0) Then
                Dim sqlins As String = String.Format("insert into trpinjam2 (nobukti,kd_gudang,kd_barang,satuan,qty,qtykecil) values('{0}','{1}','{2}','{3}',{4},{5})", tbukti.Text.Trim, _
    ...
    Private Sub hapus()
        ...
        Try
            ...
            Else
                ...
                Dim sqldel As String = String.Format("delete from trpinjam2 where noid={0}", noid)
                ...
                If DateValue(tglmuat_old) <> DateValue(ttgl_mt.EditValue) Then
                    Clsmy.Insert_HistBarang(cn, sqltrans, tbukti.Text.Trim, tglmuat_old, kdgudang, kdbar, qtykecil, 0, "Pinjam Barang", "-", "BE XXXX XX")
                Else
                    Clsmy.Insert_HistBarang(cn, sqltrans, tbukti.Text.Trim, ttgl_mt.EditValue, kdgudang, kdbar, qtykecil, 0, "Pinjam Barang", "-", "BE XXXX XX")
                End If
                'End If
...
langsung:
    ...
    Private Sub insertview()
        ...
        orow("tgl_keluar") = ttgl_mt.EditValue
    ...
    Private Sub updateview()
        ...
        dv(position)("tgl_keluar") = ttgl_mt.EditValue
    ...
    Private Sub btadd_Click(sender As System.Object, e As System.EventArgs) Handles btadd.Click
        ...
        Dim fs As New fpinjam3 With {.StartPosition = FormStartPosition.CenterParent, .WindowState = FormWindowState.Normal, .addstat = True, .position = 0, .dv = dv1, .jenis = "pinjam"}
        fs.ShowDialog(Me)
