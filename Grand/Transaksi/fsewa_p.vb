...
Public Class fsewa_p
    ...
    Private Sub opendata()
        ...
        Dim sql As String = String.Format("SELECT trsewa3.nobukti, trsewa3.tanggal, trsewa3.tanggal1, trsewa3.tanggal2, trsewa3.nobukti_sw,ms_toko.kd_toko, ms_toko.nama_toko, ms_toko.alamat_toko, trsewa3.netto, trsewa3.sbatal,trsewa3.jmlbayar " & _
            "FROM  trsewa3 INNER JOIN trsewa ON trsewa3.nobukti_sw = trsewa.nobukti INNER JOIN ms_toko ON trsewa.kd_toko = ms_toko.kd_toko where trsewa3.tanggal >='{0}' and  trsewa3.tanggal <='{1}'", convert_date_to_eng(tglperiod1), convert_date_to_eng(tglperiod2))
    ...
    Private Sub cari()
        ...
        sql = String.Format("SELECT trsewa3.nobukti, trsewa3.tanggal, trsewa3.tanggal1, trsewa3.tanggal2, trsewa3.nobukti_sw,ms_toko.kd_toko, ms_toko.nama_toko, ms_toko.alamat_toko, trsewa3.netto, trsewa3.sbatal,trsewa3.jmlbayar " & _
            "FROM  trsewa3 INNER JOIN trsewa ON trsewa3.nobukti_sw = trsewa.nobukti INNER JOIN ms_toko ON trsewa.kd_toko = ms_toko.kd_toko where trsewa3.tanggal >='{0}' and  trsewa3.tanggal <='{1}'", convert_date_to_eng(tglperiod1), convert_date_to_eng(tglperiod2))

        Select Case tcbofind.SelectedIndex
            Case 0 ' nobukti
                sql = String.Format("{0} and trsewa3.nobukti like '%{1}%'", sql, tfind.Text.Trim)
            Case 1
                sql = String.Format("{0} and trsewa3.nobukti_sw like '%{1}%'", sql, tfind.Text.Trim)
            Case 2 ' tgl
                ...
                sql = String.Format("{0} and trsewa3.tanggal='{1}'", sql, convert_date_to_eng(tfind.Text.Trim))
    ...
    Private Sub hapus()
        ...
        Dim sql As String = String.Format("update trsewa3 set sbatal=1,alasan_batal='{0}' where nobukti='{1}'", alasan.ToUpper, dv1(Me.BindingContext(bs1).Position)("nobukti").ToString)
        ...
        Dim sqlhsewa As String = String.Format("delete from hsewa where nobukti='{0}' and nobukti2='{1}'", dv1(Me.BindingContext(bs1).Position)("nobukti_sw").ToString, dv1(Me.BindingContext(bs1).Position)("nobukti").ToString)
        ...
        Try
            ...
            Clsmy.InsertToLog(cn, "btsewa_p", 0, 0, 1, 0, dv1(Me.BindingContext(bs1).Position)("nobukti").ToString, dv1(Me.BindingContext(bs1).Position)("tanggal").ToString, sqltrans)
    ...
    Private Function cekkembali() As Boolean
        ...
        Try
            ...
            Dim sql = String.Format("select nobukti from trpengembalian_sw where nobukti_sw='{0}' and sbatal=0", dv1(Me.BindingContext(bs1).Position)("nobukti_sw").ToString)
    ...
    Private Sub cekbatal_onserver()
        ...
        Try
            ...
            Dim sql As String = String.Format("select sbatal,jmlbayar from trsewa3 where nobukti='{0}'", dv1(bs1.Position)("nobukti").ToString)
            Dim cmd As OleDbCommand = New OleDbCommand(sql, cn)
    ...
    Private Sub tsdel_Click(sender As System.Object, e As System.EventArgs) Handles tsdel.Click
        ...
        If cekkembali() = True Then
            MsgBox("Barang sudah kembali..", vbOKOnly + vbInformation, "Informasi")
            Return
        End If
    ...
    Private Sub tsadd_Click(sender As System.Object, e As System.EventArgs) Handles tsadd.Click
        Using fkar2 As New fsewa_p2 With {.StartPosition = FormStartPosition.CenterParent, .dv = dv1, .addstat = True, .position = 0}
            fkar2.ShowDialog()
        End Using
    End Sub
    ...
    Private Sub tsedit_Click(sender As System.Object, e As System.EventArgs) Handles tsedit.Click
        ...
        Using fkar2 As New fsewa_p2 With {.StartPosition = FormStartPosition.CenterParent, .dv = dv1, .addstat = False, .position = bs1.Position}
            fkar2.ShowDialog()
        End Using
    ...
    Private Sub tsprint_Click(sender As System.Object, e As System.EventArgs) Handles tsprint.Click
        ...
        Using fkar2 As New fpr_sewa_p With {.nobukti = nobukti}
            fkar2.ShowDialog(Me)
        End Using
