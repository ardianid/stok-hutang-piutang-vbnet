...
Public Class fbeli_cust3

    Public dv As DataView
    Public position As Integer
    Public addstat As Boolean

    Public tglbalik As String
    ...
    Private Sub cek_nopolspm(ByVal cn As OleDbConnection)
        ...
        Dim sqlspm As String = String.Format("select trspm.nopol from trturun_br inner join trspm on trturun_br.nobukti_spm=trspm.nobukti " & _
        "where trspm.kd_sales='{0}' and trturun_br.tgl_turun='{1}'", kdsales, convert_date_to_eng(tglbalik))

        Dim cmdspm As OleDbCommand = New OleDbCommand(sqlspm, cn)
        Dim drdspm As OleDbDataReader = cmdspm.ExecuteReader

        If drdspm.Read Then
            If Not drdspm(0).ToString.Equals("") Then
                tgudang.EditValue = drdspm(0).ToString
            End If
        End If
        drdspm.Close()
